...
| **C — Đã xác minh chủ**    | PUBLISHED, **owned**     | Vendor chứng minh kiểm soát vật lý  | **OTP + on-site proof** (giấy phép optional) | Toàn quyền quản lý info + badge "Đã xác minh"                           |
...
- **Customer:** sinh viên — xem, đóng góp, đề xuất sửa, đánh giá, report, kháng cáo.
...
```
no-owner --claim approved / Vendor đăng ký mới / request-access grant/timeout hoặc dispute transfer--> owned
...
## HF-2 — Đăng ký địa điểm mới (Vendor, auto-own)

- **Actor:** Vendor · phụ: M1, M4, Admin. **Priority:** SHOULD. **Tầng kết quả:** C (owned).
- **Trigger:** "Đăng ký địa điểm mới" trong Vendor area. **Pre:** Vendor login + SĐT verified. **Post:** Location SUBMITTED (đánh dấu Vendor) → approve → auto-own.
...
4. Xác nhận → **SUBMITTED** (nguồn Vendor) → Admin duyệt.
5. Admin approve → **PUBLISHED + auto-gán owner = Vendor** (tầng C), badge xác minh; **M3** báo.

**AF**

- **AF2.1** Trùng địa điểm chưa chủ → gợi ý dùng **Claim (HF-3)** thay vì tạo mới.
- **AF2.2** Vendor **không** nộp on-site proof → chỉ tạo ở **tầng B (no-owner)**, phải claim sau (BR-60).

**EF**

- **EF2.1** Thiếu trường/ảnh → chặn. **EF2.2** Accuracy kém → kéo pin (BR-42). **EF2.3** On-site proof không đạt → không auto-own, hạ về tầng B.

## HF-3 — Claim địa điểm + Xác minh

- **Actor:** Vendor (claim), Admin (xét) · phụ: SMS (OTP), Storage. **Priority:** MUST.
- **Trigger:** "Claim địa điểm" trên địa điểm PUBLISHED & no-owner. **Pre:** Vendor login + SĐT verified; địa điểm no-owner; **không** có claim/request-access PENDING (BR-61). **Post:** owner gán Vendor (APPROVED) / REJECTED.
...
1. Vendor mở địa điểm no-owner → "Claim".
...
8. Approve **chỉ khi** OTP verified **AND** proof khớp (BR-45) → set **owner = Vendor**, badge xác minh, **APPROVED**; **M3** báo Vendor.
...
- **AF3.4** Reject + lý do → **REJECTED**; **M3** báo; gửi lại = **claim mới, không ghi đè** (BR-46); Vendor có thể **kháng cáo** (HF-6).

**EF**

- **EF3.1** Địa điểm **đã có chủ** khi mở claim → chặn → chuyển hướng **Request-Access (HF-4)**.
- **EF3.2** OTP sai/hết hạn → thử lại; quá số lần → tạm khóa (BR-03).
- **EF3.3** Thiếu on-site proof → chặn submit (BR-14).
- **EF3.4** Đang có claim/request-access PENDING → chặn (BR-61).
- **EF3.5** GPS yếu → geotag lệch → Admin dựa **mã + timestamp + biển hiệu** thay vì chỉ tọa độ.
...
**Claim & ownership:** BR-14 claim bắt buộc OTP + on-site proof · BR-15 giấy phép optional, verify = kiểm soát vật lý · BR-28 1 owner/địa điểm · BR-29 Vendor-submitted duyệt → auto-own · BR-30 đổi tên/địa chỉ → duyệt lại, public giữ info cũ · BR-45 approve claim chỉ khi OTP verified + proof khớp · BR-46 reject claim → giữ record cũ, claim mới riêng.
...
**Suggest-edit & trùng lặp:** BR-57 routing: claimed→Vendor inbox, no-owner→Admin; **không voting** · BR-58 nghi-trùng (cảnh báo) ≠ confirmed-duplicate (ẩn) · BR-59 ghi khoảng cách pin↔thiết bị, lệch xa → cờ mềm · BR-60 Vendor đăng ký mới **bắt buộc on-site proof** để auto-own · BR-62 nhiều DN cùng địa chỉ nếu khác tên + biển riêng; 1 DN nhiều dịch vụ → 1 địa điểm.
...
| mật khẩu                 | ≥ 8 ký tự                                       |
| SĐT                      | định dạng VN; Vendor verified                   |
...
| -------------------------------------- | ----------------------------------- | -------------------------------------------------- |
| Địa điểm duyệt/từ chối                 | người submit                        | kết quả (+ link gốc nếu trùng)                     |
| Claim duyệt/từ chối                    | Vendor                              | kết quả + lý do                                    |
