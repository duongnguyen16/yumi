...
export function AppealDetailDrawer({
  ...
  const selectedDecision = decisions.find((option) => option.value === decision) ?? decisions[0];
      ...
      <Stack spacing={3}>
        <Box>
          ...
          <Stack
            ...
            sx={{ mt: 0.75, justifyContent: 'space-between', alignItems: { sm: 'flex-start' }, gap: 1 }}
          >
            <Box sx={{ minWidth: 0 }}>
              <Typography sx={{ fontWeight: 700, fontSize: 20, lineHeight: 1.3 }}>
                {user?.fullName ?? user?.email ?? 'Không rõ người gửi'}
              </Typography>
              <Typography sx={{ color: tokens.color.textMuted, fontSize: 13, mt: 0.25 }}>
            ...
            <Chip
              ...
              sx={{
                flexShrink: 0,
                bgcolor: item?.status === 'PENDING' ? tokens.color.pendingBg : tokens.color.sage,
                color: item?.status === 'PENDING' ? tokens.color.pendingText : tokens.color.sageText,
        ...
        <Section title="Quyết định bị kháng cáo" icon={<GavelOutlinedIcon />}>
          ...
          <Typography sx={{ color: tokens.color.textSecondary, fontSize: 14, lineHeight: 1.6, mt: 0.75 }}>
            {item?.originalDecisionReason ?? 'Quyết định gốc không ghi nhận lý do.'}
          </Typography>
          <Typography sx={{ color: tokens.color.textMuted, fontSize: 12, mt: 1 }}>
            Thời điểm quyết định: {item?.originalDecidedAt ? new Date(item.originalDecidedAt).toLocaleString('vi-VN') : '—'}
          </Typography>
        </Section>

        <Section title="Nội dung kháng cáo" icon={<DescriptionOutlinedIcon />}>
          <Typography sx={{ fontSize: 15, lineHeight: 1.65, color: tokens.color.textPrimary }}>
          ...
          {(item?.additionalEvidenceFiles?.length ?? 0) > 0 ? (
            <Stack spacing={1}>
              {(item?.additionalEvidenceFiles ?? []).map((file, index) => (
                ...
                >
                  <DescriptionOutlinedIcon sx={{ color: tokens.color.orange, fontSize: 20 }} />
        ...
        {!readOnly ? (
          <Box>
            <Typography sx={{ fontWeight: 700, fontSize: 16 }}>Chọn hướng xử lý</Typography>
            <Typography sx={{ color: tokens.color.textSecondary, fontSize: 13, mt: 0.5, mb: 1.5 }}>
              Chọn một kết quả sau khi đã đối chiếu quyết định gốc và bằng chứng mới.
            </Typography>
            <Stack spacing={1.25}>
        ...
        {!readOnly ? (
          <TextField
            ...
            slotProps={{
              htmlInput: { 'aria-label': 'Lý do xử lý kháng cáo' },
              formHelperText: {
        ...
        {readOnly ? (
          <Section title="Kết quả xử lý" icon={<CheckCircleOutlinedIcon />} tone="success">
            ...
            <Typography sx={{ color: tokens.color.textMuted, fontSize: 12, mt: 1 }}>
              Xử lý lúc: {item?.adminDecision?.decidedAt ? new Date(item.adminDecision.decidedAt).toLocaleString('vi-VN') : '—'}
            </Typography>
          </Section>
        ) : null}
  ...
  return (
    ...
    >
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        <Typography sx={{ fontWeight: 700, color: tokens.color.textPrimary, lineHeight: 1.35 }}>
          {option.label}
        </Typography>
        <Typography sx={{ color: tokens.color.textSecondary, fontSize: 13, lineHeight: 1.45, mt: 0.35 }}>
          {option.description}
        </Typography>
      </Box>
      <CheckCircleOutlinedIcon
        sx={{ color: selected ? accent : tokens.color.border, flexShrink: 0, mt: 0.25 }}
      />
    </ButtonBase>
  );
}
