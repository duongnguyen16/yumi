...
export function DisputeDetailDrawer({
  ...
  return (
    <DetailDrawer
      ...
      width={520}
      preview={
        <GavelOutlinedIcon sx={{ fontSize: 64, color: tokens.color.orange }} />
      }
      footer={
        readOnly ? (
      ...
      <Stack spacing={2.5}>
        ...
        <Stack direction={{ xs: "column", sm: "row" }} spacing={1.5}>
          <Party
            ...
            user={a}
            files={item?.evidenceA ?? []}
          />
          <Party
            title="Vendor B · Người yêu cầu"
            user={b}
            files={item?.evidenceB ?? []}
          />
        </Stack>
        {!readOnly && (
          <Box>
...
function Party({
  ...
  title: string;
  user: DecisionUser | null;
  files: DecisionEvidence[];
}) {
