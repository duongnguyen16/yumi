...
export class AppealSourceService {
  ...
  private async user(id: Types.ObjectId): Promise<AppealSourceResult> {
    ...
    const log = await this.audit('users', id, `update_user_status:${UserStatus.BANNED}`);
    const decidedAt = log ? this.createdAt(log) : item.updatedAt;
    if (!decidedAt) return this.fail(404, 'Không tìm thấy quyết định gốc');
    return this.ok('users', id, decidedAt, log?.actorId, log?.reason);
  }
