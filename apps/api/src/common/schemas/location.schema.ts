...
import { GeoPoint, GeoPointSchema, ImageAsset, ImageAssetSchema } from './common.embedded';
...
export class Location {
  ...
  @Prop({ required: true, trim: true, minlength: 3, maxlength: 100, index: true })
  name: string;
  ...
  @Prop({ type: String, enum: LocationStatus, default: LocationStatus.SUBMITTED, index: true })
  ...
  @Prop({ type: [{ type: MongooseSchema.Types.ObjectId, ref: 'SubCategory' }], default: [], index: true })
  subCategoryIds: Types.ObjectId[];

  @Prop({ type: [{ type: MongooseSchema.Types.ObjectId, ref: 'Tag' }], default: [], index: true })
