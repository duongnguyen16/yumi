export enum UserRole {
  CUSTOMER = 'CUSTOMER',
  VENDOR = 'VENDOR',
  ADMIN = 'ADMIN',
}

export enum UserStatus {
  ACTIVE = 'ACTIVE',
  WARNED = 'WARNED',
  BANNED = 'BANNED',
}

export enum TrustLevel {
  RESTRICTED = 'RESTRICTED',
  NEW = 'NEW',
  TRUSTED = 'TRUSTED',
  BANNED = 'BANNED',
}

export enum LocationStatus {
  SUBMITTED = 'SUBMITTED',
  PUBLISHED = 'PUBLISHED',
  HIDDEN = 'HIDDEN',
  REJECTED = 'REJECTED',
  PENDING_RE_APPROVAL = 'PENDING_RE_APPROVAL',
  DELETED = 'DELETED',
}

export enum LocationSource {
  CUSTOMER = 'CUSTOMER',
  VENDOR = 'VENDOR',
  ADMIN = 'ADMIN',
}

export enum ReviewStatus {
  PUBLISHED = 'PUBLISHED',
  DELETED = 'DELETED',
  REMOVED_BY_ADMIN = 'REMOVED_BY_ADMIN',
}

export enum ClaimRequestType {
  CLAIM_EXISTING_LOCATION = 'CLAIM_EXISTING_LOCATION',
  VENDOR_NEW_LOCATION = 'VENDOR_NEW_LOCATION',
}

export enum ClaimRequestStatus {
  PENDING = 'PENDING',
  APPROVED = 'APPROVED',
  REJECTED = 'REJECTED',
  RELEASED = 'RELEASED',
  REVOKED = 'REVOKED',
}

export enum RequestAccessStatus {
  PENDING = 'PENDING',
  GRANTED = 'GRANTED',
  REJECTED = 'REJECTED',
  EXPIRED = 'EXPIRED',
  AUTO_GRANTED = 'AUTO_GRANTED',
  ESCALATED = 'ESCALATED',
}

export enum EditSuggestionStatus {
  PENDING = 'PENDING',
  APPLIED = 'APPLIED',
  DISCARDED = 'DISCARDED',
}

export enum RoutingTarget {
  VENDOR = 'VENDOR',
  ADMIN = 'ADMIN',
}

export enum DisputeStatus {
  OPEN = 'OPEN',
  RESOLVED_KEEP = 'RESOLVED_KEEP',
  RESOLVED_TRANSFER = 'RESOLVED_TRANSFER',
  RESOLVED_REVOKE = 'RESOLVED_REVOKE',
}

export enum AppealType {
  REQUEST_ACCESS_REJECTED = 'REQUEST_ACCESS_REJECTED',
  LOCATION_REJECTED = 'LOCATION_REJECTED',
  CLAIM_REJECTED = 'CLAIM_REJECTED',
  DUPLICATE_HIDDEN = 'DUPLICATE_HIDDEN',
  OWNERSHIP_REVOKED = 'OWNERSHIP_REVOKED',
  REVIEW_REMOVED = 'REVIEW_REMOVED',
  USER_BANNED = 'USER_BANNED',
  USER_WARNED = 'USER_WARNED',
}

export enum AppealStatus {
  PENDING = 'PENDING',
  ACCEPTED_TO_DISPUTE = 'ACCEPTED_TO_DISPUTE',
  OVERTURNED = 'OVERTURNED',
  UPHELD = 'UPHELD',
}

export {
  ReportReason,
  ReportRoute,
  ReportStatus,
  ReportTargetType,
} from '@wdp301/shared';

export enum OwnershipHoldStatus {
  ACTIVE = 'ACTIVE',
  EXPIRED = 'EXPIRED',
}

export enum TrustEventType {
  LOCATION_APPROVED = 'LOCATION_APPROVED',
  CORRECT_REPORT = 'CORRECT_REPORT',
  LIVE_REVIEW = 'LIVE_REVIEW',
  VIOLATING_CONTENT_REMOVED = 'VIOLATING_CONTENT_REMOVED',
  FALSE_REPORT = 'FALSE_REPORT',
  ADMIN_ADJUSTMENT = 'ADMIN_ADJUSTMENT',
}

export enum NotificationType {
  // Địa điểm
  LOCATION_APPROVED = 'LOCATION_APPROVED',
  LOCATION_REJECTED = 'LOCATION_REJECTED',

  // Claim sở hữu
  CLAIM_APPROVED = 'CLAIM_APPROVED',
  CLAIM_REJECTED = 'CLAIM_REJECTED',
  CLAIM_EVIDENCE_REQUESTED = 'CLAIM_EVIDENCE_REQUESTED',

  // Tranh chấp
  DISPUTE_RESOLVED = 'DISPUTE_RESOLVED',

  // Kháng nghị
  APPEAL_OVERTURNED = 'APPEAL_OVERTURNED',
  APPEAL_UPHELD = 'APPEAL_UPHELD',

  // Yêu cầu truy cập
  REQUEST_ACCESS_GRANTED = 'REQUEST_ACCESS_GRANTED',
  REQUEST_ACCESS_REJECTED = 'REQUEST_ACCESS_REJECTED',

  // Tài khoản
  ACCOUNT_WARNED = 'ACCOUNT_WARNED',
  ACCOUNT_BANNED = 'ACCOUNT_BANNED',

  // Đề xuất chỉnh sửa
  EDIT_SUGGESTION_APPLIED = 'EDIT_SUGGESTION_APPLIED',
  EDIT_SUGGESTION_DISCARDED = 'EDIT_SUGGESTION_DISCARDED',

  // Đánh giá
  REVIEW_REMOVED = 'REVIEW_REMOVED',

  // Hệ thống
  SYSTEM = 'SYSTEM',
}
