import {
  ...
  getLocationPublicDetails,
  getMapLocationPreview,
  locationProductsExpandedByDefault,
...
describe("map location helpers", () => {
  ...
  it("builds native directions destinations", () => {
    const target = {
      coordinates: [106.7009, 10.7769] as [number, number],
      name: "Yumi Café",
    };

    expect(buildDirectionsUrl(target, "ios")).toBe(
      "https://maps.apple.com/?daddr=10.7769%2C106.7009&dirflg=d&q=Yumi%20Caf%C3%A9",
    );
    expect(buildDirectionsUrl(target, "android")).toBe(
      "geo:10.7769,106.7009?q=10.7769,106.7009(Yumi%20Caf%C3%A9)",
    );
  });
  ...
  it("projects only public location details supported by the API", () => {
    expect(
      getLocationPublicDetails({
        address: "Thạch Hòa",
        categoryId: { name: "Thư viện" },
        description: "Không gian đọc sách",
        openingHours: "08:00-17:30",
        phone: "0901234567",
        status: "PUBLISHED",
        subCategoryIds: [{ name: "Học tập" }, { name: "Làm việc" }],
        viewCount: 42,
      }),
    ).toEqual({
      address: "Thạch Hòa",
