  ...
  return (
    <View style={{ alignItems: "flex-start", flexDirection: "row", gap: spacing[3], justifyContent: "space-between" }}>
      <View style={{ flex: 1, gap: spacing[1] }}>
        <Text variant="titleLarge" style={{ color: colors.textPrimary, fontFamily: fontFamily.bold }}>{title}</Text>
        {supportingText ? <Text style={{ color: colors.textSecondary, fontFamily: fontFamily.regular }}>{supportingText}</Text> : null}
      </View>
      {action}
    </View>
  );
...
export function ListRow({ icon, label, supportingText, value, trailing, showChevron = true, state = "default", onPress }: { icon?: IconName; label: string; supportingText?: string; value?: string; trailing?: ReactNode; showChevron?: boolean; state?: "default" | "unread" | "disabled" | "danger"; onPress?: () => void }) {
  const right = trailing
    ? () => trailing
    : value
      ? () => <Text style={{ color: state === "danger" ? colors.accentRed : colors.textSecondary, fontFamily: fontFamily.medium }}>{value}</Text>
      : showChevron
  ...
  return (
    <List.Item
      ...
      title={label}
      titleStyle={{ color: state === "danger" ? colors.accentRed : colors.textPrimary, fontFamily: state === "unread" ? fontFamily.bold : fontFamily.semibold }}
    />
  );
}
  ...
  return (
    <Portal>
      <Modal contentContainerStyle={{ backgroundColor: colors.surfaceBase, borderTopLeftRadius: radius.sheet, borderTopRightRadius: radius.sheet, gap: spacing[2], marginTop: "auto", padding: spacing[5] }} dismissable onDismiss={onDismiss} visible={visible}>
        <Text variant="titleLarge" style={{ color: colors.textPrimary, fontFamily: fontFamily.bold }}>{title}</Text>
        {message ? <Text style={{ color: colors.textSecondary, fontFamily: fontFamily.regular }}>{message}</Text> : null}
