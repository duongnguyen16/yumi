...
export default function DisputesScreen() {
  ...
  const load = useCallback(async (refresh = false) => {
    ...
    if (response.success) {
      ...
      setMessage(response.message || "Không thể lấy tranh chấp.");
    }
    ...
    <Page>
      ...
      {loading ? (
        ...
        >
          ...
          {!blockingError && items.length === 0 ? (
            <EmptyState
              icon="shield-account-outline"
              supportingText="Hồ sơ tranh chấp mà bạn tham gia sẽ xuất hiện tại đây."
              title="Chưa có tranh chấp"
            />
          ) : null}
          {!blockingError ? (
            <Stack gap={0}>
              {items.map((item) => {
                ...
                return (
                  <ActivityRow
                    ...
                    key={item._id}
                    onPress={() =>
                      router.push(`/disputes/${item._id}` as never)
                    }
