    ...
    <Page>
      <PageContent tabBarInset>
        ...
        <Stack>
          ...
          <GroupedList>
            {isVendor ? <><ListRow icon="storefront-outline" label="Quản lý địa điểm" onPress={() => router.push("/manage")} supportingText="Hiệu suất và địa điểm thuộc sở hữu" /><ListRow icon="map-marker-plus-outline" label="Đăng ký địa điểm" onPress={() => router.push(getVendorRegistrationDestination(phoneVerified))} supportingText="Thêm cơ sở kinh doanh mới" /><ListRow icon="map-marker-plus-outline" label="Đóng góp địa điểm" onPress={() => router.push(getCustomerContributionDestination())} supportingText="Gợi ý địa điểm còn thiếu như khách hàng" /></> : <ListRow icon="map-marker-plus-outline" label="Đóng góp địa điểm" onPress={() => router.push(getCustomerContributionDestination())} supportingText="Thêm một địa điểm còn thiếu trên bản đồ" />}
          </GroupedList>
        </Stack>
