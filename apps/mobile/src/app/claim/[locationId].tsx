...
export default function ClaimLocationScreen() {
  ...
  if (step === 0) {
    ...
    continueLabel = "Nhận mã xác minh";
  } else if (step === 1) {
    continueAction = confirmOtp;
    continueLabel = "Xác nhận OTP";
  }

  const metadata = siteCode ? `Mã hiện trường: ${siteCode}` : undefined;
  ...
  const noticeAction = submitted
    ? { label: "Đóng", onPress: () => router.back() }
    : undefined;
  ...
  return (
    <>
      ...
      <WizardScreen
        ...
        stepLabels={["Thông tin liên hệ", "Xác minh OTP", "Bằng chứng sở hữu"]}
        title="Xác nhận sở hữu"
      >
        {step === 0 ? (
          <Card>
            <UIStack>
              <AppText variant="headline">Bắt đầu xác minh</AppText>
              <AppText
                style={{ color: colors.textSecondary }}
                variant="subhead"
              >
                Hệ thống cấp mã hiện trường. Nếu địa điểm có số điện thoại, OTP
                sẽ được gửi tới số đó.
              </AppText>
            </UIStack>
          </Card>
        ) : null}
        {step === 1 ? (
          <FormSection
            supportingText="Nhập mã gồm 6 chữ số đã gửi tới số điện thoại của địa điểm."
        ...
        {step === 2 ? (
          <UIStack gap={spacing[4]}>
            <Card>
              <UIStack>
                <AppText
                  style={{ color: colors.accentPrimary, letterSpacing: 2 }}
                  variant="largeTitle"
                >
                  {siteCode}
                </AppText>
                <AppText
                  ...
                  variant="subhead"
                >
                  Đặt mã cạnh biển hiệu và chụp ảnh rõ ràng tại địa điểm.
                </AppText>
              </UIStack>
            </Card>
            <MediaPicker
