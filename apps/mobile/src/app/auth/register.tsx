...
export default function Register() {
  ...
  const handleSubmitForm = async () => {
    ...
    if (!validatedPassword.isValid) {
      setError(validatedPassword.message || "Mật khẩu không hợp lệ.");
      return;
    }
    ...
    if (!validatedConfirmPassword.isValid) {
      setError(
        validatedConfirmPassword.message || "Mật khẩu xác nhận không hợp lệ.",
      );
      return;
    }
    const validatedEmail = validateEmail(email);
    if (!validatedEmail.isValid) {
      setError(validatedEmail.message || "Email không hợp lệ.");
      return;
    }
    const validatedName = validateUserName(name);
    if (!validatedName.isValid) {
      setError(validatedName.message || "Họ tên không hợp lệ.");
      return;
    }
    const validatedPhone = validatePhoneNumber(phone);
    if (role === "vendor" && !validatedPhone.isValid) {
      setError(validatedPhone.message || "Số điện thoại không hợp lệ.");
      return;
    }
    setLoading(true);
    if (role === "customer") {
  ...
  const supportingText =
    step === "select-role"
      ? "Chọn loại tài khoản phù hợp với nhu cầu hiện tại của bạn."
      : step === "otp"
        ? `Nhập mã gồm 6 số đã gửi đến ${phone}.`
        : "Thông tin này được dùng để bảo vệ và cá nhân hóa tài khoản.";
    ...
    >
      ...
      >
        {step === "select-role" ? (
          <GroupedList>
            <ListRow
              ...
              supportingText="Khám phá, lưu và đóng góp địa điểm"
            />
            <ListRow
              icon="store-outline"
              label="Đối tác kinh doanh"
              onPress={() => handleSelectRole("vendor")}
              supportingText="Quản lý địa điểm và cần xác minh số điện thoại"
            />
          </GroupedList>
        ) : null}
        ...
        {step === "form" ? (
          <Stack gap={spacing[4]}>
            ...
            {role === "vendor" ? (
              <FormSection
                supportingText="Số điện thoại này được dùng để xác minh tài khoản đối tác."
                title="Xác minh"
              >
                <TextField
      ...
      <BottomActionBar>
        ...
        {step === "form" ? (
          <Button
            disabled={loading}
            label={role === "vendor" ? "Tiếp tục nhận OTP" : "Tạo tài khoản"}
            loading={loading}
