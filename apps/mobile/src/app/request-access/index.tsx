...
function getRelatedUser(item: AccessRequest, side: AccessSide) {
  if (side === "owner") {
    return typeof item.requesterId === "object" ? item.requesterId : null;
  }

  return typeof item.currentOwnerId === "object" ? item.currentOwnerId : null;
}
...
function AccessTabContent({ side }: { side: AccessSide }) {
  ...
  const load = useCallback(async () => {
    ...
    if (response.success) {
      ...
      setMessage(response.message || "Không thể lấy dữ liệu.");
    }
    setLoading(false);
  }, [side]);
  ...
  const emptySupportingText =
    side === "owner"
      ? "Yêu cầu gửi đến bạn sẽ xuất hiện tại đây."
        ...
        >
          ...
          <Stack gap={0}>
            {items.map((item) => {
              ...
              const relatedUser = getRelatedUser(item, side);
              const userLabel = relatedUser?.fullName || relatedUser?.email;
              const supportingText = userLabel || location?.address;
              ...
              return (
                <ActivityRow
                  ...
                  key={item._id}
                  onPress={() =>
                    router.push(`/request-access/${item._id}` as never)
                  }
                  status={getWorkflowStatus(item.effectiveState)}
                  supportingText={supportingText}
                  title={location?.name || "Địa điểm"}
                />
              );
            })}
          </Stack>
